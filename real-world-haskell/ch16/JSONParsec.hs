-- file: ch16/JSONParsec.hSeek
import ApplicativeParsec
import Numeric (readHex, readSigned, readFloat)

-- a JSON value must be either an object or an array
p_text :: CharParser () JValue
p_text = spaces *> text <?> "JSON text"
    where text = JObject <$> p_object <|> JArray <$> p_array

p_series :: Char -> CharParser () a -> Char -> CharParser () [a]
p_series left parser right = between (char left <* spaces) (char right) $ (parser <* spaces) `sepBy` (char ',' <* spaces)
